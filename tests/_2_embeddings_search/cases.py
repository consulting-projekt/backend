import pandas as pd

# start und dest types = ["adress", "poi_or_aoi",  "station" , None]
test_cases = [
    {
        "vars": {
            "anfrage": {
                "start": None,  # point of interest oder area of interest
                # immer eine area of interest
                "dest": "Kirche",  # point of interest oder area of interest
                "dest_aoi": "Kirchdorf-Süd"  # immer eine area of interest
            },
            "assert": {
                "target_distance2centroid": ["POINT(10.000413 53.554069)", 1000]
            }
        }
    },

    # "Wann kommt der nächste Bus in die Innenstadt?"
    {
        "vars": {
            "anfrage": {
                "start": None,

                "dest": "Innenstadt",
                "dest_aoi": None
            },
            "assert": {
                "target_distance2centroid": ["POINT(9.988080668856236 53.55184559306223)", 5000]
            }
        }
    },

    # "Zeige mir eine Verbindung von Herthastraße zu einem Restarurant in Nähe vom Hafen?"
    {
        "vars": {
            "anfrage": {
                "start": None,

                "dest": "Restarurant",
                "dest_aoi": "Hafen"
            },
            "assert": {
                "target_distance2centroid": ["POINT(9.984983833154688 53.47308499617683)", 5000]
            }
        }
    },

    # "Zeige mir eine Verbindung von Herthastraße zur Innenstadt?"
    {
        "vars": {
            "anfrage": {
                "start": None,

                "dest": "Innenstadt",
                "dest_aoi": None
            },
            "assert": {
                "target_distance2centroid":  ["POINT(9.988080668856236 53.55184559306223)", 5000]
            }
        }
    },

    # "Ich möchte vom U Mümmelmannsberg zu einem Jugendzentrum in der Nähe der Speicherstadt fahren."
    {
        "vars": {
            "anfrage": {
                "start": None,

                "dest": "Jugendzentrum",
                "dest_aoi": "Speicherstadt"
            },
            "assert": {
                "target_distance2centroid": ["POINT(10.00467865 53.54357754999999)", 5000]
            }
        }
    },

    # "Wie komme ich vom Billwerder Ring zur nächsten Behörde?"
    {
        "vars": {
            "anfrage": {
                "start": None,

                "dest": "Behörde",
                "dest_aoi": None
            },
            "assert": {
                "ziel_name_contains": ["Behörde", "Amt", "Verwaltung"],
                "target_distance2centroid": ["POINT(9.984663 53.554245)", 5000]
            }
        }
    },  # "Wie komme ich vom U Steinfurther Allee zum nächsten Jugendzentrum im Stadtteil Billstedt?"
    {
        "vars": {
            "anfrage": {
                "start": None,

                "dest": "Jugendzentrum",
                "dest_aoi": "Billstedt"
            },
            "assert": {
                "ziel_name_contains": ["Jugend"],
                # Ungefähre Koordinaten für Billstedt
                "target_distance2centroid": ["POINT(10.104508884450647 53.54136343631377)", 5000]
            }
        }
    },

    # "Zeige mir den Weg von der Kirchdorfer Straße zur Bücherhalle in der Nähe der Hafencity."
    {
        "vars": {
            "anfrage": {
                "start": None,

                "dest": "Bücherhalle",
                "dest_aoi": "Hafencity"
            },
            "assert": {
                "ziel_name_contains": ["Bücherhalle", "Bibliothek"],
                # Ungefähre Koordinaten für Hafencity
                "target_distance2centroid":  ["POINT(9.994725 53.541915)", 5000]
            }
        }
    },

    # "Verbindung von der Davidwache zum Haus der Familie am Elbstrand."
    {
        "vars": {
            "anfrage": {
                "start": "Davidwache",

                "dest": "Haus der Familie",
                "dest_aoi": "Elbstrand"
            },
            "assert": {
                "start_name_contains": ["Davidwache", "Polizei"],
                "ziel_name_contains": ["Haus der Familie"],
                # Ungefähre Koordinaten für Elbstrand
                "target_distance2centroid": ["POINT(9.96242186569669 53.557507857835716)", 5000]
            }
        }
    },

    # "Fahre von S Billwerder-Moorfleet zur Grundschule in Allermöhe."
    {
        "vars": {
            "anfrage": {
                "start": None,

                "dest": "Grundschule",
                "dest_aoi": "Allermöhe"
            },
            "assert": {
                "ziel_name_contains": ["Grundschule", "Schule"],
                # Ungefähre Koordinaten für Allermöhe
                "target_distance2centroid": ["POINT(10.160813 53.486764)", 5000]
            }
        }
    },

    # "Von Bahnhof Bergedorf zur Freiwilligen Feuerwehr in der Nähe des Curslacker Deichs."
    {
        "vars": {
            "anfrage": {
                "start": None,

                "dest": "Freiwilligen Feuerwehr",
                "dest_aoi": "Curslacker Deich"
            },
            "assert": {
                "ziel_name_contains": ["Feuerwehr"],
                # Ungefähre Koordinaten für Curslacker Deich
                "target_distance2centroid": ["POINT(9.793617541260351 53.514917074226595)", 5000]
            }
        }
    },

    # "Fahre vom Billwerder Billdeich zum Bürgerhaus im Stadtteil Wilhelmsburg."
    {
        "vars": {
            "anfrage": {
                "start": None,

                "dest": "Bürgerhaus",
                "dest_aoi": "Wilhelmsburg"
            },
            "assert": {
                "ziel_name_contains": ["Bürgerhaus"],
                # Ungefähre Koordinaten für Wilhelmsburg
                "target_distance2centroid": ["POINT(9.995909203025185 53.50033548012308)", 5000]
            }
        }
    },

    # "Wie komme ich vom Bezirksamt Bergedorf zum Krankenhaus in der Nähe vom Boberger Drift?"
    {
        "vars": {
            "anfrage": {
                "start": "Bezirksamt Bergedorf",

                "dest": "Krankenhaus",
                "dest_aoi": "Boberger Drift"
            },
            "assert": {
                "start_name_contains": ["Bezirksamt", "Bergedorf"],
                "ziel_name_contains": ["Krankenhaus"],
                # Ungefähre Koordinaten für Boberger Drift
                "target_distance2centroid": ["POINT(10.152339730730844 53.513922532693044)", 5000]
            }
        }
    },

    # "Ich möchte vom U Mümmelmannsberg zum Finanzamt in der Nähe der Mundsburg fahren."
    {
        "vars": {
            "anfrage": {
                "start": None,

                "dest": "Finanzamt",
                "dest_aoi": "Mundsburg"
            },
            "assert": {
                "ziel_name_contains": ["Finanzamt"],
                # Ungefähre Koordinaten für Mundsburg
                "target_distance2centroid": ["POINT(10.00333 53.550451)", 5000]
            }
        }
    },

    # "Zeig mir den schnellsten Weg von der Elbchaussee zum Museum am Alsterufer."
    {
        "vars": {
            "anfrage": {
                "start": None,

                "dest": "Museum",
                "dest_aoi": "Alsterufer"
            },
            "assert": {
                "ziel_name_contains": ["Museum"],
                # Ungefähre Koordinaten für Alsterufer
                "target_distance2centroid": ["POINT(10.001416935070356 53.56763769598505)", 5000]
            }
        }
    },

    # "Verbindung von S Nettelnburg zum Spielplatz im Friedrich-Frank-Bogen."
    {
        "vars": {
            "anfrage": {
                "start": None,

                "dest": "Spielplatz",
                "dest_aoi": "Friedrich-Frank-Bogen"
            },
            "assert": {
                "ziel_name_contains": ["Spielplatz"],
                # Ungefähre Koordinaten für Friedrich-Frank-Bogen
                "target_distance2centroid": ["POINT(10.178885799999998 53.49181495)", 5000]
            }
        }
    },

    # "Wie gelange ich von Billwerder-Moorfleet zum Schwimmbad in Bergedorf?"
    {
        "vars": {
            "anfrage": {
                "start": None,

                "dest": "Schwimmbad",
                "dest_aoi": "Bergedorf"
            },
            "assert": {
                "ziel_name_contains": ["Schwimmbad", "Schwimmhalle", "Bad"],
                # Ungefähre Koordinaten für Bergedorf
                "target_distance2centroid": ["POINT(10.220398 53.487293)", 5000]
            }
        }
    },
    # "Route von Neuengammer Hausdeich zum Job-Center bei der Agentur für Arbeit Harburg."
    {
        "vars": {
            "anfrage": {
                "start": None,

                "dest": "Job-Center",
                "dest_aoi": "Agentur für Arbeit Harburg"
            },
            "assert": {
                "ziel_name_contains": ["Job-Center", "Jobcenter", "Agentur für Arbeit"],
                # Ungefähre Koordinaten für Harburg
                "target_distance2centroid": ["POINT(9.978567 53.460712)", 5000]
            }
        }
    },
    # "Wie komme ich vom Rathaus Bergedorf zur Grundschule am Friedrich-Frank-Bogen?"
    {
        "vars": {
            "anfrage": {
                "start": "Rathaus Bergedorf",

                "dest": "Grundschule",
                "dest_aoi": "Friedrich-Frank-Bogen"
            },
            "assert": {
                "start_name_contains": ["Rathaus", "Bergedorf"],
                "ziel_name_contains": ["Grundschule", "Schule"],
                # Ungefähre Koordinaten für Friedrich-Frank-Bogen
                "target_distance2centroid": ["POINT(10.178885799999998 53.49181495)", 5000]
            }
        }
    }
]


def generate_tests():
    return test_cases


if __name__ == "__main__":
    # Example usage
    import ast
    test_cases = generate_tests()
    for case in test_cases:
        anfrage = case["vars"]["anfrage"]
        anfrage_fromstr = ast.literal_eval(str(anfrage).strip())
        print(f"From str: {anfrage_fromstr}")
