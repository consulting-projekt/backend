...
def raw_llm2json(raw_llm_output):
    try:
        ...
        required_fields = ["start", "start_typ", "dest", "ziel_typ"]
        ...
        if len(json_output["dest"]) != len(json_output["ziel_typ"]):
...
def calculate_score(llm_json, expected_json):
    ...
    try:
        ...
        if "dest" in expected_json and len(expected_json["dest"]) > 0:
            expected_ziels = set(expected_json["dest"])
            llm_ziels = set(llm_json["dest"])
